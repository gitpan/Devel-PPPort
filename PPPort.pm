...
POD =head1 NAME
POD 
POD __PPPORT_NAME__ - Perl/Pollution/Portability version __VERSION__
POD 
POD =head1 SYNOPSIS
POD 
POD   perl __PPPORT_NAME__ [options] [files]
POD 
...
POD 
POD This version of F<__PPPORT_NAME__> is designed to support operation with Perl
...
POD 
POD Tell F<__PPPORT_NAME__> to check for compatibility with the given
...
POD of F<__PPPORT_NAME__> if you intend to be backward compatible only
...
POD F<__PPPORT_NAME__>. Also lists if it must be explicitly requested,
...
POD F<__PPPORT_NAME__> and below which version of Perl they probably
...
POD 
POD     perl __PPPORT_NAME__ --list-provided
POD 
POD to see which API elements are provided by __PPPORT_NAME__.
POD 
POD =item *
POD 
...
POD also deprecated. You can safely use the supported API, as F<__PPPORT_NAME__>
...
POD more C<#define>s in your source code before the inclusion of F<__PPPORT_NAME__>.
POD 
...
POD F<__PPPORT_NAME__> on your source code. See the next section for
POD details.
POD 
POD =head1 EXAMPLES
POD 
POD To verify whether F<__PPPORT_NAME__> is needed for your module,
POD whether you should make any changes to your code, and whether any
POD special defines should be used,  F<__PPPORT_NAME__> can be run as
POD as Perl script to check your source code. Simply say:
POD 
POD     perl __PPPORT_NAME__
POD 
...
POD =head1 BUGS
POD 
POD If this version of F<__PPPORT_NAME__> is causing failure during
...
POD 
POD If F<__PPPORT_NAME__> was generated using the latest version of
...
IS_NUMBER_GREATER_THAN_UV_MAX|||p
IS_NUMBER_INFINITY|||p
IS_NUMBER_IN_UV|||p
IS_NUMBER_NAN|||p
IS_NUMBER_NEG|||p
IS_NUMBER_NOT_INT|||p
...
PERL_LONG_MIN|||p
PERL_MAGIC_arylen|||p
PERL_MAGIC_backref|||p
PERL_MAGIC_bm|||p
PERL_MAGIC_collxfrm|||p
PERL_MAGIC_dbfile|||p
PERL_MAGIC_dbline|||p
PERL_MAGIC_defelem|||p
PERL_MAGIC_envelem|||p
PERL_MAGIC_env|||p
PERL_MAGIC_ext|||p
PERL_MAGIC_fm|||p
PERL_MAGIC_glob|||p
PERL_MAGIC_isaelem|||p
PERL_MAGIC_isa|||p
PERL_MAGIC_mutex|||p
PERL_MAGIC_nkeys|||p
PERL_MAGIC_overload_elem|||p
PERL_MAGIC_overload_table|||p
PERL_MAGIC_overload|||p
PERL_MAGIC_pos|||p
PERL_MAGIC_qr|||p
PERL_MAGIC_regdata|||p
PERL_MAGIC_regdatum|||p
PERL_MAGIC_regex_global|||p
PERL_MAGIC_shared_scalar|||p
PERL_MAGIC_shared|||p
PERL_MAGIC_sigelem|||p
PERL_MAGIC_sig|||p
PERL_MAGIC_substr|||p
PERL_MAGIC_sv|||p
PERL_MAGIC_taint|||p
PERL_MAGIC_tiedelem|||p
PERL_MAGIC_tiedscalar|||p
PERL_MAGIC_tied|||p
PERL_MAGIC_utf8|||p
PERL_MAGIC_uvar_elem|||p
PERL_MAGIC_uvar|||p
PERL_MAGIC_vec|||p
...
PERL_QUAD_MIN|||p
PERL_REVISION|||p
PERL_SCAN_ALLOW_UNDERSCORES|||p
PERL_SCAN_DISALLOW_PREFIX|||p
PERL_SCAN_GREATER_THAN_UV_MAX|||p
PERL_SCAN_SILENT_ILLDIGIT|||p
PERL_SHORT_MAX|||p
PERL_SHORT_MIN|||p
PERL_SUBVERSION|||p
PERL_UCHAR_MAX|||p
...
for $filename (@files) {
  ...
  for $func (keys %API) {
    ...
    if ($c =~ /\b(?:Perl_)?($match)\b/) {
      $file{uses_replace}{$1}++ if $1 eq $revreplace{$func};
